...
collect_computer_info() {
    ...
    # 네트워크 정보 (첫 번째 이더넷 인터페이스 사용)
    IP_ADDRESS=$(ip addr show | grep 'inet ' | grep -v '127.0.0.1' | head -n1 | awk '{print $2}' | cut -d/ -f1)
    MAC_ADDRESS=$(ip link show | grep 'link/ether' | head -n1 | awk '{print $2}')
    ...
    DESCRIPTION="[시스템 정보] 제조사: $(sudo dmidecode -s system-manufacturer) 모델: $MODEL 시리얼번호: $SERIAL 운영체제: $OS_VERSION 프로세서: $CPU_INFO 메모리: $MEMORY_SIZE 디스크: $DISK_SIZE [네트워크 정보] 이름: $(ip link show | grep 'link/ether' | head -n1 | awk '{print $2}') MAC: $MAC_ADDRESS (ETHERNET) IP: $IP_ADDRESS"
    
    # JSON 데이터 생성 (한 줄로)
    json_data="{\"mac_addresses\":[{\"mac_address\":\"$MAC_ADDRESS\",\"interface_type\":\"ETHERNET\",\"equipment\":null}],\"name\":\"노트북 컴퓨터\",\"manufacturer\":\"$(sudo dmidecode -s system-manufacturer)\",\"model_name\":\"$MODEL\",\"equipment_type\":\"LAPTOP\",\"serial_number\":\"$SERIAL\",\"description\":\"$DESCRIPTION\",\"status\":\"RENTED\",\"acquisition_date\":\"$CURRENT_DATE\"}"
